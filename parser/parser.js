...
const parseExpression = (tokenList, index = 0) => {
    ...
    while (index < tokenList.length) {
        ...
        if (
            isType(currT, TokenType.variable) &&
            isType(nextT, TokenType.bracketLeft)
        ) {
            ...
            isType(currT, TokenType.variable) &&
            isType(nextT, TokenType.parenthesesLeft)
        ) {
    ...
    while (stackOperator.length !== 0) {
        ...
        if (isType(operator, TokenType.not)) {
            ...
            if (
                ...
                isType(right.type, AstType.ExpressionCall)
            ) {
                // 类方法调用
...
const tks = toTokens(str);
console.log("code:", str);
console.log("tokens :", tks);
